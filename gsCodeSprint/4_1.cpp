...
int query(int node, int start, int end, int v) {
  if(tree[node] == v ) return tree[node];
  if(start == end) return arr[start].second;

  int left = 2*node;
  int right = 2*node+1;
  int mid = (start+end)/2;

  if(v <= tree[left]) return query(left, start, mid, v);
  else return query(right, mid+1, end, v);
}
